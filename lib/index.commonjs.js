...
function addABI(abiArray) {
  if (Array.isArray(abiArray)) {
    ...
    abiArray.map(function (abi) {
      if (abi.name) {
        const signature = web3.utils.sha3(abi.name + "(" + abi.inputs.map(typeToString).join(",") + ")");
...
function removeABI(abiArray) {
  if (Array.isArray(abiArray)) {
    ...
    abiArray.map(function (abi) {
      if (abi.name) {
        const signature = web3.utils.sha3(abi.name + "(" + abi.inputs.map(function (input) {
          return input.type;
        }).join(",") + ")");
...
function decodeLogs(logs) {
  return logs.filter(log => log.topics.length > 0).map(logItem => {
    ...
    if (method) {
      ...
      const decodedData = web3.eth.abi.decodeParameters(dataTypes, logData.slice(2)); // Loop topic and data to get the params
      ...
      method.inputs.map(function (param) {
        ...
        if (param.type === "address") {
          ...
          if (decodedP.value.length > 42) {
            // @ts-ignore
            const toRemove = decodedP.value.length - 42; // @ts-ignore

            const temp = decodedP.value.split("");
            temp.splice(2, toRemove); // @ts-ignore
        ...
        if (param.type === "uint256" || param.type === "uint8" || param.type === "int") {
          ...
          if ( // @ts-ignore
          typeof decodedP.value === "string" && // @ts-ignore
